...
fn serialize_state(state: &ExtruderState) -> StatePrompt {
    ...
    prompt.push_str(&format!(
        "- Current Motor Torque: {:.2} N·m (Absolute Limit: 15.0 N·m)\n",
    ...
    prompt.push_str(&format!(
        "- Thermal Strain (Die Temperature): {:.2} K (Limit: 360.0 K)\n\n",
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let args = Args::parse();
    println!(
        "Initializing DUMSTO-LLM Framework for model: {}",
        args.model
    );
    ...
    for i in 0..args.episodes {
        ...
        match action_result {
            Ok(action) => {
                println!(
                    "    LLM Action Proposed: ΔTorque={:.2}, ΔFlow={:.2} (Confidence: {:.2})",
